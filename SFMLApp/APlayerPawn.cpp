...
sf::Int32 APlayerPawn::CreatePawn(float x, float y)
{
	...
	if (!TxIdle.loadFromFile(SpritePath + Name + "\\Idle.png")) {
		std::cout << "Idle >> ";
		return EEndStatus::FileLoadFaled;
	}

	if (!TxIdle.loadFromFile(SpritePath + Name + "\\Move.png")) {
		std::cout << "Move >> ";
		return EEndStatus::FileLoadFaled;
	}
	

	if (!TxIdle.loadFromFile(SpritePath + Name + "\\Jump.png")) {
		std::cout << "Jump >> ";
		return EEndStatus::FileLoadFaled;
	}

	ASprite.setTexture(TxIdle);
	ASprite.setTextureRect(sf::IntRect(0, 0, 32, TxIdle.getSize().y));
...
void APlayerPawn::ChangePose()
{
	...
	switch (Action) {
	case EActionList::Move_Left || EActionList::Move_Right:	
		// Смена текущего кадра в зависимости от времени
		ASprite.setTexture(TxMove);
		CurrPoseFrame = (CurrPoseFrame + 1) % (TxMove.getSize().x / 32);

		ASprite.setTextureRect(sf::IntRect(32 * CurrPoseFrame, 0, 32 * XDirection, TxMove.getSize().y));
		break;
		/*
	...
	default:
		...
		ASprite.setTextureRect(sf::IntRect(16 + (16 * XDirection), 0, 32, TxIdle.getSize().y));
		CurrPoseFrame = 0;
	...
	if (!bOnEarth) {
		ASprite.setTexture(TxJump);

		ASprite.setTextureRect(sf::IntRect(0, 0, 32, TxJump.getSize().y));
	}
	Clock.restart();
}
